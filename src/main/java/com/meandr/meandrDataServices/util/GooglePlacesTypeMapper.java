 ...
 *
 * All keys are uppercase — toGoogleTypes normalizes input to uppercase before
 * lookup.
 *
 * Strategy: - Use specific Google types where they exist (state_park, beach,
 * marina etc.) - For broad/noisy types (natural_feature), rely on keyword
 * search instead - Multiple types in list = searchNearby returns places
...
public class GooglePlacesTypeMapper {
    ...
    private static final Map<String, List<String>> TYPE_MAP = Map.ofEntries(
            ...
            Map.entry("PEAK", List.of("mountain_peak", "tourist_attraction")),
            Map.entry("SCENIC_OVERLOOK", List.of("observation_deck", "scenic_spot", "tourist_attraction")),
            Map.entry("RIVER_ACCESS", List.of("river", "park", "tourist_attraction")),
            Map.entry("NATURE_RESERVE", List.of("nature_preserve", "wildlife_refuge", "national_park", "park")),
            Map.entry("WINERY", List.of("winery")),
            Map.entry("LAKE", List.of("lake")),
            Map.entry("ISLAND", List.of("island")),
            Map.entry("WOODS", List.of("woods")),
            Map.entry("VISITOR_CENTER", List.of("visitor_center"))
    );
